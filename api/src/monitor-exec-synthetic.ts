...
function extractCodeFromLexicalState(serializedState: SerializedEditorState): string {
  let code = '';
  function traverse(node: SerializedLexicalNode) {
    if (node.type === 'code' || node.type === 'code-highlight') { // Handle code blocks specifically if needed
    ...
    } else if (node.type === 'paragraph' || node.type === 'root' || node.type === 'line-break') {
        if (node.type === 'paragraph' && code.length > 0 && !code.endsWith('\n')) {
...
export default class MonitorExecSynthetic extends WorkerEntrypoint<CloudflareEnv> {
  ...
  private async _executeCheck(payload: SyntheticExecPayload) {
    ...
    const monitor = await db
      .select({ id: SyntheticMonitorsTable.id, name: SyntheticMonitorsTable.name })
      .from(SyntheticMonitorsTable)
      .where(eq(SyntheticMonitorsTable.id, monitorId))
      .then(takeFirstOrNull)

    if (!monitor) {
      console.error(`Synthetic Monitor [${monitorId}] does not exist. Cannot execute check.`)
    ...
    try {
      ...
      if (!scriptObject) {
        throw new Error(`Script state object not found in R2 for key: ${scriptStateKey}`);
      }
      ...
      console.log(`Fetched and parsed script state [${scriptStateKey}] for monitor [${monitorId}]`);
      ...
      console.error(`Failed to fetch or parse script state for [${monitorId}]:`, error);
      await this.recordCheckResult(db, monitorId, "failure", 0, `Script retrieval/parsing failed: ${error instanceof Error ? error.message : String(error)}`);
      return;
    }

    // Ensure executableCode is not null (already handled by catch, but for TS)
    if (!executableCode) {
        console.error(`Executable code is unexpectedly null for [${monitorId}] after parsing.`);
        await this.recordCheckResult(db, monitorId, "failure", 0, "Internal error: Executable code null after parsing.");
        return;
    }
    
    // 3. Execute via Browser Rendering using EXTRACTED code
    let checkResult: { statusOutcome: "success" | "failure"; durationMs: number; errorMessage?: string } = {
    ...
    try {
      console.log(`Launching browser for [${monitorId}] with runtime [${runtime}] and timeout [${timeoutSeconds}s]`);
      ...
      const userScriptFunction = new Function('page', 'context', 'browser', executableCode);
      ...
      console.log(`Synthetic check for [${monitorId}] completed successfully in ${durationMs}ms.`);
      ...
      console.error(`Synthetic check for [${monitorId}] failed after ${durationMs}ms:`, errorMsg);
      checkResult = { statusOutcome: "failure", durationMs, errorMessage: errorMsg };
      ...
      try {
          ...
          console.error(`Error closing browser after script failure for [${monitorId}]:`, closeError);
      }
    }
    ...
    console.log(`Synthetic check finished for [${monitorId}]. Outcome: ${checkResult.statusOutcome}`);
  }
  ...
  private async recordCheckResult(
    ...
    try {
        ...
        console.error(`Failed to record check result for [${monitorId}]:`, dbError);
    }
  }
} 