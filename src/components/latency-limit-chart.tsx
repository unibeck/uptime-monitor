...
import { type ChartConfig, ChartContainer, ChartTooltip } from "@/registry/new-york-v4/ui/chart"
import { ChartTooltipContent } from "@/registry/new-york-v4/ui/chart"
import { Skeleton } from "@/registry/new-york-v4/ui/skeleton"
import { useEffect, useId, useMemo, useRef, useState } from "react"
import { CartesianGrid, Line, LineChart, Rectangle, ResponsiveContainer, Tooltip, XAxis, YAxis } from "recharts"
...
export function LatencyLimitChart({
  ...
  useEffect(() => {
    const fetchLatencyData = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          console.error(
            `Failed to fetch latency data: ${response.statusText}`
          )
          setData([])
          return
        }
        const responseData = await response.json() as { timestamp: string; responseTime: number }[]
  ...
  const gradientStops = useMemo(() => {
    ...
    return stops
      ...
      .filter((stop, index, self) =>
          index === 0 || index === self.length -1 || stop.color !== self[index - 1].color || stop.color !== self[index + 1].color
      );
              ...
              formatter={(value, name) => {
                if (name === "responseTime" && typeof value === "number") {
                  if (value >= 1000) {
                    ...
                    return <p className="text-xs"><span className="font-semibold">{seconds.toFixed(2)}</span> s</p>
                  }
                  return <p className="text-xs"><span className="font-semibold">{value.toFixed(0)}</span> ms</p>
                }
