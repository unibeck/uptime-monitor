...
export const useSyntheticDataTableStore = create<SyntheticDataTableState>(
  (set, get) => ({
    // Initial state
    data: [],
    ...
    fetchSyntheticMonitors: async () => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/synthetic-monitors?${params.toString()}`,
        )
        if (!response.ok) {
        ...
        toast.error("Failed to load synthetic monitors.", DEFAULT_TOAST_OPTIONS)
