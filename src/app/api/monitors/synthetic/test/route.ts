...
function extractCodeFromLexicalStateLocal(serializedState: SerializedEditorState): string {
  let code = '';
...
const syntheticTestSchema = z.object({
  ...
  scriptContent: z.string().min(1, { message: "Script JSON state cannot be empty" }),
});
...
export const POST = createRoute
  ...
  .handler(async (_request, context) => {
    ...
    const { runtime, timeoutSeconds, scriptContent: scriptStateString } = context.body;

    console.log(`Starting synthetic test with runtime [${runtime}], timeout [${timeoutSeconds}s]`);
    ...
    try {
        ...
        return NextResponse.json(
            { statusOutcome: "failure", durationMs: 0, errorMessage: `Failed to parse script: ${parseError instanceof Error ? parseError.message : 'Invalid format'}` },
            { status: BAD_REQUEST }
        );
    }

    let testResult: { statusOutcome: "success" | "failure"; durationMs: number; errorMessage?: string };
    ...
    try {
      ...
      console.error("Synthetic test script execution failed after", durationMs, "ms:", errorMsg);
      testResult = { statusOutcome: "failure", durationMs, errorMessage: errorMsg };
      try {
        // Need a way to ensure browser instance is accessible or handle potential errors if env.BROWSER is not the instance
        // For now, assuming direct launch means we might not have a shared instance to close this way.
        // A more robust approach might involve managing browser instances if needed, but direct launch/close should work.
        // await env.BROWSER.close(); // This might error if called on the binding, not the instance
      } catch (closeError) {
          console.error("Error potentially closing browser after test script failure:", closeError);
      }
    }
